...
local function Events( event )
  if event:GetName() == "player_death" and checkbox_buybot:GetValue() then
    if client.GetPlayerIndexByUserID( event:GetInt( 'userid' ) ) ~= client.GetLocalPlayerIndex() then
      ...
      if k_armor:GetValue() then buymisc = "buy vesthelm;"
        end
      if armor:GetValue() then buymisc = "buy vest;"
      end
      if nades:GetValue() then buymisc = buymisc.."buy hegrenade;buy molotov;buy smokegrenade;buy flashbang;buy flashbang;"
      end
      if buybot_zeus:GetValue() then buymisc = buymisc.."buy taser;"
      end
      if defuser:GetValue() then buymisc = buymisc.."buy defuser;"
      end
      if primary > 0 then buyguns = buyguns..primary_w[primary]..";"
      end
      if secondary > 0 then buyguns = buyguns..secondary_w[secondary]..";"
      end
      client.Command(buyguns, false)
      client.Command(buymisc, false)
    end
  end
end
